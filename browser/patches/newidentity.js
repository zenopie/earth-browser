...
ChromeUtils.defineLazyGetter(this, "NewIdentityButton", () => {
  ...
  class NewIdentityImpl {
    async run() {
      await this.newReticulumIdentity();
      await this.clearState();
      this.broadcast();
      // Open a fresh blank tab instead of restarting the browser
      const browser = window.gBrowser;
      const tabsToRemove = [...browser.tabs];
      browser.addWebTab("about:blank");
      tabsToRemove.forEach(tab => browser.removeTab(tab));
    }

    async newReticulumIdentity() {
      logger.info("Generating new Reticulum identity");
      try {
        const resp = await fetch("http://_earth.ret/new-identity", { method: "POST" });
        const data = await resp.json();
        if (data.ok) {
          logger.info(`New Reticulum identity: ${data.identity_hash}`);
        }
      } catch (e) {
        logger.warn("Could not reach earthproxy for identity reset", e);
      }
    }
